...
def main_get():
    return render_template ('index.html', developer_name = 'Ruslan',not_valid = False)
@app.route('/', methods = ['POST'])
def main_post():
    alpha = request.form ["number"]
    if not alpha.isdigit():
        return render_template ('index.html', developer_name = 'Ruslan',not_valid = True) 
    
    number =int(alpha)
    if (0 > number):
        return render_template ('index.html', developer_name = 'Ruslan',not_valid = True)
    return render_template ('result.html', milliseconds=number, result= convert (number), developer_name = 'Ruslan')
